Hey, I'm Malys 👋
Aspiring Full-Stack Developer · Linux enthusiast · Building tools I actually use
I run Garuda Linux with i3wm and spend my time customizing my desktop, writing scripts, and learning backend development. Currently working through Python and TypeScript — building real projects along the way.

🔨 What I'm Working On

i3-keybindings-cheatsheet — A live, searchable, editable cheatsheet for i3 keybindings. Parses your config, serves a local web UI, integrates with polybar. Zero dependencies.

🧰 Tech & Tools
Languages    Python · TypeScript · Bash · HTML/CSS/JS
Environment  Garuda Linux · i3wm · Polybar
Learning     Backend development · Full-stack fundamentals

📫 Find Me

💻 Right here on GitHub
☕ Sponsor my work


<sub>I build tools that solve my own problems — then share them.</sub>
